...
class _ConversationListItemState extends State<ConversationListItem> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.conversation.finishedAt != null &&
        widget.conversation.finishedAt!.isAfter(memorizedAt)) {
      memorizedAt = widget.conversation.finishedAt!;
    }
    int seconds = (DateTime.now().millisecondsSinceEpoch -
            memorizedAt.millisecondsSinceEpoch) ~/
        1000;
    isNew = 0 < seconds && seconds < 60; // 1m
    if (isNew) {
      _conversationNewStatusResetTimer?.cancel();
      _conversationNewStatusResetTimer =
    ...
    return Consumer<ConversationProvider>(builder: (context, provider, child) {
      return GestureDetector(
        onTap: () async {
          MixpanelManager().conversationListItemClicked(
              widget.conversation, widget.conversationIdx);
          context
              .read<ConversationDetailProvider>()
              .updateConversation(widget.conversationIdx, widget.date);
          String startingTitle = context
              .read<ConversationDetailProvider>()
              .conversation
              .structured
              .title;
            ...
            child: ClipRRect(
              ...
              child: Dismissible(
                ...
                confirmDismiss: (direction) {
                  bool showDeleteConfirmation = SharedPreferencesUtil()
                ...
                : Row(
                    ...
                    children: [
                      ...
                      if (widget.conversation.transcriptSegments.isNotEmpty &&
                          _getConversationDuration().isNotEmpty)
                        Padding(
...
class ConversationNewStatusIndicator extends StatefulWidget {
  ...
  @override
  State<ConversationNewStatusIndicator> createState() =>
      _ConversationNewStatusIndicatorState();
}

class _ConversationNewStatusIndicatorState
    extends State<ConversationNewStatusIndicator>
